...
const downloadFile = async (url, fileName, albumDir) => {
    ...
    try {
        ...
        // Usar Fetch + writeFile (funciona em mais casos)
        try {
        ...
        try {
            ...
            if (coverUrl) {
                try {
                    ...
                    await downloadFile(coverUrl, coverFileName, albumDir);
                    console.log('✅ Capa do álbum baixada com sucesso');
                } catch (coverError) {
                    console.warn('⚠️ Erro ao baixar capa do álbum (continuando):', coverError.message);
                    coverFileName = null;
                }
            }
            ...
            const albumDownload = {
                ...
                coverFileName: coverFileName, // Para acesso offline
                albumDir: albumDir,
        ...
        try {
            ...
            if (album) {
                try {
                    ...
                    console.log(`✅ Pasta do álbum deletada: ${album.albumDir}`);
                } catch (deleteError) {
                    // Se a pasta não existe, continuar mesmo assim (pode ter sido deletada manualmente)
                    console.warn(`⚠️ Erro ao deletar pasta (continuando): ${deleteError.message}`);
                }
                
                // Remover do estado e metadados independentemente
                const updated = downloads.filter(d => d.albumId !== albumId);
                setDownloads(updated);
                await saveMetadata(updated);
                console.log(`✅ Álbum removido dos metadados: ${album.title}`);
            }
        } catch (error) {
        ...
        try {
            ...
            if (album) {
                ...
                try {
                    ...
                    console.log(`✅ Música deletada: ${fileName}`);
                } catch (deleteError) {
                    // Se o arquivo não existe, continuar mesmo assim
                    console.warn(`⚠️ Erro ao deletar arquivo (continuando): ${deleteError.message}`);
                }
                ...
                setDownloads(updated);
                await saveMetadata(updated);
                console.log(`✅ Música removida dos metadados`);
            }
        } catch (error) {
