...
import { Play, Heart, Share2, Download, Plus, BadgeCheck, Copy, X, Disc, ThumbsUp, Video, Loader } from 'lucide-react';
...
const AlbumPage = () => {
     ...
     const { downloadAlbumDirect, downloadProgress, cacheAlbumMetadata, cacheArtistMetadata, cacheImage } = useDownloadManager();
     ...
     // Debounce hooks - DEVEM ficar antes de qualquer return condicional
     const debouncedPlay = useDebounce(async (song) => {
     ...
     const handlePlayAlbumDebounced = useDebounce(async () => {
         if (albumSongs.length > 0 && album) {
             const isMobile = window.innerWidth < 768;
             playSong(albumSongs[0], albumSongs);
             if (isMobile) {
                 setTimeout(() => setShowMobilePlayer(true), 0);
             }
             if (album.id) {
                 ...
                 if (!error) {
                     ...
                     await supabase.from('albums').update({ play_count: (album.play_count || 0) + 1 }).eq('id', album.id);
                     setAlbum(prev => ({ ...prev, play_count: (prev.play_count || 0) + 1 }));
                 }
             }
         }
     ...
     const handleMobilePlayerToggleDebounced = useDebounce((song) => {
         const isMobile = window.innerWidth < 768;
         if (isMobile) {
             playSong(song, albumSongs);
             setTimeout(() => setShowMobilePlayer(true), 0);
     ...
     useEffect(() => {
        ...
        const loadAlbum = async () => {
            ...
            // Cachear metadados automaticamente
             if (isMounted && supabaseAlbum && artistData) {
                 cacheAlbumMetadata(supabaseAlbum);
                 cacheArtistMetadata(artistData);
                 if (supabaseAlbum.cover_url) {
                     cacheImage(supabaseAlbum.cover_url);
                 }
             }
    ...
    const handlePlayAlbum = handlePlayAlbumDebounced;

    const handleDownloadAlbum = async () => {
        // Se é app instalado (PWA), usar download direto para IndexedDB
        if (isPWA) {
            return handleDownloadAlbumForApp();
        }

        // Comportamento normal de navegador (ZIP)
        if (album.archiveUrl) {
            toast({
    ...
    const handleDownloadAlbumForApp = async () => {
        if (!album || !albumSongs || albumSongs.length === 0) {
            toast({
                title: 'Erro',
                description: 'Álbum ou músicas não encontrados',
                variant: 'destructive'
            });
            return;
        }
        ...
        try {
            ...
            // Baixar direto para IndexedDB
            const success = await downloadAlbumDirect(album, albumSongs);

            if (success) {
                toast({
                    title: 'Sucesso!',
                    description: `${album.title} está pronto para ouvir offline`,
                    variant: 'default'
                });
            } else {
                toast({
                    title: 'Aviso',
                    description: 'Algumas músicas não foram baixadas. Tente novamente.',
                    variant: 'destructive'
                });
            }
        } catch (error) {
            console.error('Erro ao baixar para app:', error);
            toast({
                title: 'Erro',
                description: 'Não foi possível baixar o álbum',
                variant: 'destructive'
            });
        } finally {
    ...
    const handleMobilePlayerToggle = handleMobilePlayerToggleDebounced;
                ...
                <div className="max-w-7xl mx-auto px-4 py-6">
                    ...
                    <div className="hidden md:flex items-center gap-4">
                        ...
                        <Button
                            ...
                            className={`text-white px-8 h-12 text-base font-bold shadow-lg ${
                                isDownloadingApp
                        ...
                        >
                            {isDownloadingApp ? (
                                <>
                                    ...
                                    {downloadProgress[`album-${album.id}`] || 0}%
                                </>
                            ) : (
                                <>
                        ...
                        {/* DownloadButton só aparece no mobile quando não é PWA */}
                        {!isPWA && (
