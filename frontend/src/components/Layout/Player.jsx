...
const Player = () => {
    ...
    const lastOpenedAlbumIdRef = useRef(null);
    ...
    // Abrir player mobile fullscreen ao mudar de álbum (apenas mobile)
    useEffect(() => {
        if (!currentSong) return;
        
        const albumId = currentSong.albumId || currentSong.album_id;
        
        // Só abrir se for um ÁLBUM DIFERENTE do último que abriu o player
        if (window.innerWidth < 768 && !isClosingPlayer && lastOpenedAlbumIdRef.current !== albumId) {
    ...
    }, [currentSong?.albumId, currentSong?.album_id, isClosingPlayer]);
                            ...
                            <div className="flex-1 max-w-2xl">
                                <div className="flex items-center justify-center gap-3 mb-1">
                                    ...
                                    <button
                                        ...
                                        className="w-10 h-10 rounded-full bg-red-600 hover:bg-red-700 text-white shadow-lg flex items-center justify-center"
                                    >
                                ...
                                <div className="flex-1 max-w-2xl">
                                    <div className="flex items-center justify-center gap-3 mb-1">
                                        <button
                                            ...
                                            className={`h-8 w-8 flex items-center justify-center rounded-full focus:outline-none active:bg-white/20 ${isShuffle ? 'text-white' : 'text-white/60'}`}
                                        >
                                            <Shuffle className="w-3.5 h-3.5" />
                                        </button>
                                        <button
                                            onClick={handlePrevious}
                                            className="h-8 w-8 flex items-center justify-center rounded-full text-white focus:outline-none active:bg-white/20"
                                        >
                                            <SkipBack className="w-4 h-4" />
                                        </button>
                                        <button
                                            onClick={togglePlay}
                                            className="w-10 h-10 rounded-full bg-white text-red-600 shadow-lg flex items-center justify-center focus:outline-none active:scale-95 active:bg-gray-100"
                                        >
                                            {isPlaying ? <Pause className="w-5 h-5" fill="currentColor" /> : <Play className="w-5 h-5 ml-0.5" fill="currentColor" />}
                                        </button>
                                        <button
                                        ...
                                        <button
                                            ...
                                            className={`h-8 w-8 flex items-center justify-center rounded-full relative focus:outline-none active:bg-white/20 ${repeatMode !== 'off' ? 'text-white' : 'text-white/60'}`}
                                        >
                                            <Repeat className="w-3.5 h-3.5" />
                                            {repeatMode === 'one' && (
                                                <span className="absolute text-xs font-bold">1</span>
