...
// Registrar Service Worker para PWA e solicitar armazenamento persistente
const ServiceWorkerRegister = () => {
  useEffect(() => {
    ...
    // Solicitar armazenamento persistente (importante para iOS/Safari)
    const requestPersistentStorage = async () => {
      if (navigator.storage && navigator.storage.persist) {
        const isPersisted = await navigator.storage.persisted();
        if (!isPersisted) {
          const granted = await navigator.storage.persist();
          console.log('Armazenamento persistente:', granted ? 'concedido' : 'negado');
        } else {
...
// Redirecionar PWA para home ao instalar
const PWAHomeRedirect = () => {
  ...
  useEffect(() => {
    ...
    if (isPWA && !loading) {
      // Se abrir em rota de login/signup na PWA, redirecionar para home
      const pathname = window.location.pathname;
      
      if (pathname === '/login' || pathname === '/cadastrar' || pathname === '/reset-password') {
        console.log('PWA abriu em login, redirecionando para home...');
