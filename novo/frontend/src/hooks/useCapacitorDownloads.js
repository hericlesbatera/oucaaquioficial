...
export const useCapacitorDownloads = (onSongDownloadStart) => {
    ...
    const deleteDownloadedAlbum = useCallback(async (albumId) => {
        try {
            console.log('[Downloads] Iniciando exclusão do álbum:', albumId);
            const album = downloads.find(d => d.albumId === albumId);
            if (!album) {
                console.warn('[Downloads] Álbum não encontrado para exclusão:', albumId);
                return;
            }
            if (!useIndexedDBFallback) {
                try {
                    await deleteDirectory(`${DOWNLOADS_DIR}/${album.albumDir}`);
                    console.log('[Downloads] Pasta do álbum excluída:', album.albumDir);
                } catch (err) {
                    console.warn('[Downloads] Erro ao excluir pasta:', err.message);
                }
            }
            if (useIndexedDBFallback) {
                try {
                    await deleteFromIndexedDB(albumId);
                    console.log('[Downloads] Álbum excluído do IndexedDB:', albumId);
                } catch (err) {
                    console.warn('[Downloads] Erro ao excluir do IndexedDB:', err.message);
                }
            }
            const updated = downloads.filter(d => d.albumId !== albumId);
            console.log('[Downloads] Estado atualizado, álbuns restantes:', updated.map(a => a.albumId));
            setDownloads(updated);
            if (!useIndexedDBFallback) {
                try {
                    await saveMetadata(updated);
                    console.log('[Downloads] Metadados salvos após exclusão.');
                } catch (err) {
                    console.warn('[Downloads] Erro ao salvar metadados:', err.message);
                }
            }
        } catch (error) {
            console.error('[Downloads] Erro ao deletar álbum:', error);
            throw error;
        }
    }, [downloads]);
